...
def kmeans_color(features, k, num_iters=500):
    ...
    ### YOUR CODE HERE
    features_vector = np.reshape(features, (N, features.shape[2]))

    idxs = np.random.choice(N, size=k, replace=False)
    centers = features_vector[idxs]

    for n in range(num_iters):
        distances = cdist(features_vector, centers)
        new_assignments = np.argmin(distances, axis=1)
        ...
        for i in range(k):
            cluster_points = features_vector[assignments == i]
            if len(cluster_points) > 0:
                centers[i] = np.mean(cluster_points, axis=0)
...
def findpeak(data, idx, r):
    ...
    while shift.all() > t:
        ...
        if len(nearby_indices) > 0:
            mean_vector = np.mean(dataT[nearby_indices], axis=0)
            shift = np.linalg.norm(mean_vector - data_point)

            data_point = mean_vector
            data_pointT = data_point.reshape(1, 3)
        else:
            shift = np.array([0]) 
            break
        ### END YOUR CODE
