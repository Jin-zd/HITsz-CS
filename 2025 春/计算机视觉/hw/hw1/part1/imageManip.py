...
def rotate2d(point, theta):
    ...
    rotated_point = np.array([x_rotated, y_rotated])
...
def rotate_image(input_image, theta):
    ...
    for out_row in range(input_rows):
        for out_col in range(input_cols):
            y = out_row - center_row
            x = out_col - center_col
            
            point = np.array([x, y])
            rotated_point = rotate2d(point, -theta) 

            in_row = int(rotated_point[1] + center_row)
            in_col = int(rotated_point[0] + center_col)
            
            if (0 <= in_row < input_rows) and (0 <= in_col < input_cols):
                output_image[out_row, out_col] = input_image[in_row, in_col]
    ### END YOUR CODE
