...
void outputStudentLinkedList(StudentLinkedListNode *head) {
    ...
    // 请你实现遍历链表的逻辑
    // TODO
    for (StudentLinkedListNode *p = head; p != NULL; p = p->next) {
...
StudentLinkedListNode *reverseStudentLinkedList(StudentLinkedListNode *head) {
    ...
    if (head == NULL || head->next == NULL) {
        ...
        StudentLinkedListNode *reversed = reverseStudentLinkedList(head->next);
        head->next->next = head;
        head->next = NULL;
        return reversed;
    }
...
StudentLinkedListNode *findCrossBeginNode(StudentLinkedListNode *class1, StudentLinkedListNode *class2) {
    ...
    // 请不要简单地通过结点结构体的成员来判断。
    // TODO
    StudentLinkedListNode *front1 = class1;
    StudentLinkedListNode *front2 = class2;
